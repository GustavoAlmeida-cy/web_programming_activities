...
function identify_num() {
  ...
  if (first_num === second_num) {
    return (result.value = "The numbers are equals!");
  }

  // Check if the numbers value.
  if (first_num > second_num) {
    return (result.value = `The ${first_num} is greater than ${second_num}!`);
  } else {
    return (result.value = `The ${second_num} is greater than ${first_num}!`);
  }
}
